 ...
 * หน้าจัดการผู้เล่น/สมาชิกสำหรับ Agent
 */
const MemberManagement = () => {
  ...
  // Handle Credit Click
  const handleCreditClick = (member) => {
    setSelectedMember(member);
    setCreditFormData({
      amount: 0,
      action: 'add',
  ...
  const handleCreditHistoryClick = async (member) => {
    setSelectedMember(member);
    ...
    try {
      const result = await memberService.getCreditHistory(member._id);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-bg-card rounded-lg p-4 mb-6 border border-border-default shadow-md">
        ...
        <div className="mt-4 text-sm text-text-muted">
          พบ {filteredMembers.length} รายการ
      ...
      <div className="bg-bg-card rounded-lg overflow-hidden border border-border-default shadow-md">
        <DataTable
          ...
          loading={loading}
          emptyMessage="ไม่พบข้อมูลผู้เล่น"
        />
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          try {
            ...
            await memberService.toggleStatus(selectedMember._id, newStatus);
            toast.success(
              `${newStatus === 'active' ? 'เปิดใช้งาน' : 'ระงับ'}ผู้เล่น ${
        ...
        title={
          selectedMember?.status === 'active' ? 'ยืนยันการระงับผู้เล่น' : 'ยืนยันการเปิดใช้งาน'
        }
        message={`คุณต้องการ${
          selectedMember?.status === 'active' ? 'ระงับ' : 'เปิดใช้งาน'
        }ผู้เล่น "${selectedMember?.username}" หรือไม่?`}
        confirmText="ยืนยัน"
        cancelText="ยกเลิก"
        type={selectedMember?.status === 'active' ? 'danger' : 'warning'}
      ...
      <Modal
        ...
        onClose={closeAllModals}
        title={`ประวัติเครดิต - ${selectedMember?.name || ''}`}
        size="large"
      >
        {/* Date Filter */}
...
const CreateMemberModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`สร้างผู้เล่น ${formData.username} สำเร็จ`);
      fetchMembers();
      onClose();
    } catch (err) {
      toast.error(parseErrorMessage(err));
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="สร้างผู้เล่นใหม่" size="lg">
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end space-x-3 mt-6">
          ...
          >
            ...
            <span>{submitLoading ? 'กำลังสร้าง...' : 'สร้างผู้เล่น'}</span>
          </button>
        </div>
      </form>
    </Modal>
...
const EditMemberModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`แก้ไขข้อมูลผู้เล่น ${selectedMember.username} สำเร็จ`);
      fetchMembers();
      onClose();
    } catch (err) {
      toast.error(parseErrorMessage(err));
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="แก้ไขข้อมูลผู้เล่น" size="lg">
      <form onSubmit={handleSubmit}>
        <div className="space-y-4">
          {/* Username (Read-only) */}
          <div>
...
const AdjustCreditModal = ({
  ...
  const validateForm = () => {
    ...
    if (
      ...
      creditFormData.amount > (selectedMember?.credit || 0)
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitLoading(true);
      await memberService.adjustCredit(
        selectedMember._id,
        creditFormData.amount,
        creditFormData.action
      );
      toast.success(
        `${creditFormData.action === 'add' ? 'เพิ่ม' : 'ลด'}เครดิตผู้เล่น ${
          selectedMember.username
        } สำเร็จ`
      );
      fetchMembers();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="ปรับเครดิตผู้เล่น" size="md">
      <form onSubmit={handleSubmit}>
        <div className="space-y-4">
          {/* Member Info */}
            ...
            <div className="bg-bg-light-cream border border-primary-gold/50 p-4 rounded-lg">
              <p className="text-sm text-text-secondary">
                ...
                <span className="font-semibold text-primary-dark-gold">
                  {formatCurrency(
                    creditFormData.action === 'add'
                      ? selectedMember.credit + creditFormData.amount
                      : selectedMember.credit - creditFormData.amount
                  )}
                </span>
              </p>
