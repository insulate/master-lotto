...
/**
 * Agent Commission Management Page
 * หน้าจัดการค่าคอมมิชชันของเอเย่นต์สำหรับ Master
 */
const AgentCommissionPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch agent data
      const agentsResponse = await agentService.getAll();
      const foundAgent = agentsResponse.data.agents.find((a) => a._id === agentId);

      if (!foundAgent) {
        toast.error('ไม่พบข้อมูลเอเย่นต์');
        navigate('/master/agents');
        return;
      }
      ...
      const rates = types.map((type) => {
        const existingRate = foundAgent.commission_rates?.find(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await agentService.update(agentId, updateData);
      toast.success(`อัพเดทค่าคอมมิชชันเอเย่นต์ ${agent.username} สำเร็จ`);
      navigate('/master/agents');
    } catch (err) {
      toast.error(parseErrorMessage(err));
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <PageHeader
        ...
        subtitle={`เอเย่นต์: ${agent.name} (${agent.username})`}
        leftContent={
          <button
            onClick={() => navigate('/master/agents')}
      ...
      <div className="bg-bg-card rounded-lg p-6 mb-6 border border-border-default shadow-md">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <p className="font-semibold text-text-primary">{agent.username}</p>
          </div>
          <div>
